...
import api.player_status_hydrator as player_status_hydrator
from api.player_status_hydrator import PlayerStatusHydrator
...
def Test_Hydrate_TestCachedLandings_ExpectStatusInserted(
   ...
   player_id = 7
   landing = { 'playerId': player_id, 'isActive': False }
   ...
   assert inserted == [ [ PlayerStatus( player_id, False ) ] ]
...
def Test_Hydrate_TestExistingStatus_ExpectSkipped(
   ...
   db_path = str( tmp_path / 'skaters.sqlite' )
   fetched: list[ bool ] = []
   SkaterSeasonStore.insert_rows( [ _season( 1 ) ], db_path=db_path )
